...
class Decensor(QtCore.QThread):
    ...
    def load_model(self):
        ...
        if self.model is None :
            ...
            if self.is_mosaic==False:
                checkpoint_path = "./models/bar.pth"
            else:
                checkpoint_path = "./models/mosaic.pth"
    ...
    def decensor_image_variation(self, ori, colored, variant_number, file_name):
        ...
        for region_counter, region in enumerate(regions, 1):
            ...
            crop_img_array = torch.tensor(crop_img_array).permute(0,3,1,2).float()  # N,C,H,W
            mask_array = torch.tensor(mask_array).permute(0,3,1,2).float()
            crop_img_array = crop_img_array.to(self.device)
            mask_array = mask_array.to(self.device)
            pred_img_array,_,_,_ = self.model(crop_img_array, crop_img_array, mask_array)
            pred_img_array = pred_img_array.permute(0,2,3,1).detach().cpu().numpy() # N,H,W,C
